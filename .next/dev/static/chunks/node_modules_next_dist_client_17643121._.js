...
function onCaughtError(thrownValue, errorInfo) {
    ...
    if ("TURBOPACK compile-time truthy", 1) {
        ...
        const matches = (_ref = // example 1: at Page (http://localhost:3000/_next/static/chunks/pages/index.js?ts=1631600000000:2:1)
...
function writeDynamicTreeResponseIntoCache(now, task, fetchStrategy, response, serverData, entry, couldBeIntercepted, canonicalUrl, routeIsPPREnabled) {
    ...
    const staleTimeSeconds = typeof ((_serverData_rp = serverData.rp) === null || _serverData_rp === void 0 ? void 0 : _serverData_rp[1]) === 'number' ? serverData.rp[1] : parseInt((_response_headers_get = response.headers.get(_approuterheaders.NEXT_ROUTER_STALE_TIME_HEADER)) !== null && _response_headers_get !== void 0 ? _response_headers_get : '', 10);
...
function convertServerPatchToFullTreeImpl(baseRouterState, baseData, treePatch, dataPatch, segmentPath, index) {
    ...
    for(const parallelRouteKey in baseTreeChildren){
        ...
        const childBaseSeedData = baseSeedDataChildren !== null ? (_baseSeedDataChildren_parallelRouteKey = baseSeedDataChildren[parallelRouteKey]) !== null && _baseSeedDataChildren_parallelRouteKey !== void 0 ? _baseSeedDataChildren_parallelRouteKey : null : null;
...
function restoreReducer(state, action) {
    ...
    const restoredNextUrl = (_ref = (0, _computechangedpath.extractPathFromFlightRouterState)(treeToRestore)) !== null && _ref !== void 0 ? _ref : restoredUrl.pathname;
    const now = Date.now();
    const accumulation = {
        scrollableSegments: null,
        separateRefreshUrls: null
...
async function fetchServerAction(state, nextUrl, param) {
    ...
    const [location1, _redirectType] = (redirectHeader === null || redirectHeader === void 0 ? void 0 : redirectHeader.split(';')) || [];
    let redirectType;
...
function serverActionReducer(state, action) {
    ...
    return fetchServerAction(state, nextUrl, action).then(async (param)=>{
        let { revalidationKind, actionResult, actionFlightData: flightData, actionFlightDataRenderedSearch: flightDataRenderedSearch, actionFlightDataCouldBeIntercepted: flightDataCouldBeIntercepted, redirectLocation, redirectType } = param;
...
const publicAppRouterInstance = {
    ...
    (href, options)=>{
        ...
        const prefetchKind = (_ref = options === null || options === void 0 ? void 0 : options.kind) !== null && _ref !== void 0 ? _ref : _routerreducertypes.PrefetchKind.AUTO;
...
function createInitialRouterState(param) {
    let { navigatedAt, initialFlightData, initialCanonicalUrlParts, initialRenderedSearch, location } = param;
    var _ref;
